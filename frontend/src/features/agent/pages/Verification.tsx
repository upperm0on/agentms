...
export function AgentVerification(props: AppProps) {
  ...
  return (
    <div className="page narrow-page">
      ...
      <FormSection title="Submitted evidence" description="Documents are visible only to authorized reviewers."><div className="document-list">{agent.documents.map((doc) => <div key={doc.id}><FileCheck2 /><span><strong>{doc.title}</strong><small>Uploaded document</small></span><Badge tone="success">Received</Badge></div>)}</div><button className="btn secondary"><Upload size={16} />Add another document</button></FormSection>
    </div>
  )
}
